...
class Funcionario{
	...
	public function logaFuncionario($dado){
		 ...
		 $this->senha = sha1($dado['senha']);
			...
			try{
				...
				if($cst->rowCount() == 0) {
					header('location: /loginRoupariaCarioca/?login=error');
				} else {
					session_start();
					$rst = $cst->fetch();
					$_SESSION['logado'] = "sim";
					$_SESSION['func'] = $rst['idFuncionario'];
					header("location: /loginRoupariaCarioca/pages/home.php");
				}
	...
	public function sairFuncionario(){
		session_destroy();
		header ('location: http://localhost/loginRoupariaCarioca');
	}
